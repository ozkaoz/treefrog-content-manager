...
| **Content Manager SD target (3A)** | docs/PLAN.md (Phase 3A), docs/ARCHITECTURE.md §7 (SD target), `profiles/treefrogui/sd_markers.json` | `treefrog-manager/src-tauri/src/sd_target.rs` (VolumeInfo, TargetAnalysis, list_volumes, analyze_target, calculate_space, validate_destination_path, check_case_collision), `treefrog-manager/python/treefrog/sd_target.py` (mirror), `treefrog-manager/src-tauri/src/sd.rs` (detect + write probe, tri-state writable/healthy), `treefrog-manager/src-tauri/src/planner.rs` (single source), `treefrog-manager/src/components/SdCardPanel.tsx` (native pickFolder, volume info, analysis, dry-run), `treefrog-manager/src/App.tsx` (SD Card tab) | `treefrog-manager/tests/test_sd_target.py` (17 tests: valid/incomplete/unknown/inaccessible, removable-volume, ROM/media/BIOS/LGPT detection, logical units, hashing via planner, space ok/insufficient, destination validation, case-collision, integration, deterministic, zero writes, video/BIOS/LGPT space), `cargo check`, `npm run build` | DEC-2026-08-30-01 |
| **Content Manager audit hardening (canonical plan model)** | `docs/ai/AUDIT_2026-08-31_TREEFROG_MANAGER.md` | `treefrog-manager/src-tauri/src/paths.rs` (ONE destination validator: `resolve_validated_destination` — absolute/UNC/drive/`..`/empty/ADS/reserved/illegal + containment), `treefrog-manager/src-tauri/src/deploy.rs` (safe writer secure-by-itself, real video conversion pipeline), `treefrog-manager/src-tauri/src/planner.rs` (`effective_action` via lib, `apply_resolutions_ctx` collision-safe keep_both), `treefrog-manager/src-tauri/src/bios_catalog.rs` (bios.json-derived), `treefrog-manager/src-tauri/src/db.rs` (SQLite migrations + deployment records), `treefrog-manager/src-tauri/src/sd.rs` (tri-state detect), `treefrog-manager/src-tauri/src/sd_target.rs` (stable_id = volume GUID+serial), `treefrog-manager/src-tauri/src/lib.rs` (`resolve_plan`, `app_version`, `get_content_counts`, BIOS-as-PlanEntry), Python mirrors in `treefrog-manager/python/treefrog/` | Rust `cargo test` (paths/security/BIOS escape/keep_both/space/conversion), `tests/test_audit_invariants_2026_08_31.py`, `tests/test_archive_security_fixtures.py`, `tests/test_video_conversion_deploy.py` (real ffmpeg), `.github/workflows/validate.yml` | DEC-2026-08-31-01 |
