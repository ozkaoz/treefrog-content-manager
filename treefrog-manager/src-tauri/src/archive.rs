...
impl Default for Limits {
    fn default() -> Self {
        Self { max_entries: 1024, max_expansion_bytes: 1024*1024*1024, max_depth: 1, max_total_files_per_job: 10000, max_compression_ratio: 100.0 }
    }
}
...
fn check_entry_safety(name: &str, entry: &zip::read::ZipFile, _limits: &Limits) -> Result<(), ArchiveError> {
    if name.starts_with('/') || name.starts_with('\\') {
        return Err(ArchiveError::Safety(format!("absolute path entry: {}", name)));
    }
    if is_windows_drive_absolute(name) {
        return Err(ArchiveError::Safety(format!("windows drive-letter absolute entry: {}", name)));
    }
    if Path::new(name).is_absolute() {
        return Err(ArchiveError::Safety(format!("absolute path entry (pure): {}", name)));
    }
    // traversal
    ...
    if let Some(mode) = entry.unix_mode() {
        ...
        if ftype == 0o120000 {
            return Err(ArchiveError::Safety(format!("symlink hazard (unix_mode): {}", name)));
        }
        if ftype != 0 && ftype != 0o100000 && ftype != 0o040000 && ftype != 0o120000 {
            return Err(ArchiveError::Safety(format!("hardlink/unsafe file type hazard: {} mode={:o}", name, ftype)));
        }
    }
    ...
    if name.contains(':') && !is_windows_drive_absolute(name) {
        ...
        return Err(ArchiveError::Safety(format!("hardlink/ADS hazard (colon in name): {}", name)));
    }
    Ok(())
}
...
pub fn inspect_zip(path: &Path, limits: &Limits) -> Result<Vec<ArchiveEntry>, ArchiveError> {
    ...
    if za.len() > limits.max_entries {
        return Err(ArchiveError::Safety(format!("archive exceeds max entries {} > {}", za.len(), limits.max_entries)));
    }
    let mut total: u64 = 0;
    let mut out = Vec::new();
    ...
    for i in 0..za.len() {
        ...
        if entry.is_dir() {
            let norm = name.replace('\\', "/").to_lowercase().trim_end_matches('/').to_string();
            if seen.contains_key(&norm) {
                return Err(ArchiveError::Collision(format!("collision: duplicate normalized path {} vs {}", name, seen[&norm])));
            }
            seen.insert(norm, name.clone());
            out.push(ArchiveEntry { name: name.clone(), is_dir: true, size: 0, compressed_size: entry.compressed_size() as u64, crc: entry.crc32() });
            continue;
        }
        total = total.saturating_add(entry.size());
        if total > limits.max_expansion_bytes {
            return Err(ArchiveError::Safety(format!("archive exceeds max expansion {} > {}", total, limits.max_expansion_bytes)));
        }
        if entry.compressed_size() > 0 && entry.size() > 1024*1024 {
            let ratio = entry.size() as f64 / entry.compressed_size() as f64;
            if ratio > limits.max_compression_ratio {
                return Err(ArchiveError::Safety(format!("excessive compression ratio {:.1} for {}", ratio, name)));
            }
        }
        let norm = name.replace('\\', "/").to_lowercase();
        if seen.contains_key(&norm) {
            return Err(ArchiveError::Collision(format!("collision: normalized path duplicate {} vs {}", name, seen[&norm])));
        }
        seen.insert(norm, name.clone());
        out.push(ArchiveEntry { name: name.clone(), is_dir: false, size: entry.size(), compressed_size: entry.compressed_size() as u64, crc: entry.crc32() });
    }
    Ok(out)
}
...
pub fn inspect_archive(path: &Path, limits: &Limits) -> Result<Vec<ArchiveEntry>, ArchiveError> {
    ...
    match ext.as_str() {
        ...
        ".7z" | ".rar" => Err(ArchiveError::Unsupported(format!("archive handler not available for {} (stub)", ext))),
...
pub fn is_archive_runtime_payload(path: &Path, inner_entries: &[ArchiveEntry], profile: &crate::profile::LoadedProfile) -> bool {
    ...
    for e in inner_entries {
        ...
        if [".cue",".bin",".chd",".m3u",".sfc",".nes",".gba",".gb",".gbc",".md",".sms",".gg"].contains(&inner_ext.as_str()) {
            has_known_inner = true;
            break;
        }
    }
...
pub fn safe_join(dest_root: &Path, dest_dir: &str, file_name: &str) -> anyhow::Result<PathBuf> {
    if file_name.contains("..") || Path::new(file_name).is_absolute() || is_windows_drive_absolute(file_name) {
